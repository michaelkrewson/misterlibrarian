...
OG_IMAGE_ES = f"{SITE_URL}{BASE}/img/og-default.es.png"
...
def _chrome(active="", lang="en", lang_href=None):
    ...
    brand = ('<a class="brand" href="%s">%s<span class="wm">%s</span></a>'
             % (u["front"], MARK_SVG.replace("__ACCENT__", ACCENT), u["brand"]))
    return ('<header class="hsm">'
            ...
            '%s%s</div></header>'
            % (hublink, brand, search, hamburger, _nav(active, lang), langlink))
...
def _social_tags(title, desc, image="", lang="en"):
    """The og:image + Twitter-card block a shared link unfurls from — ported
    from build_finance.py's `_social_tags` (and build_notebook.py's bilingual-
    aware copy of it). This publication's `<head>` never had it: every entry/
    shell template hardcoded `twitter:card content="summary"` with no
    `og:image`/`twitter:image` at all, so a link shared to X/Slack/iMessage
    rendered as a bare text card even for an entry with a real hero photo —
    caught 2026-09-23 on the Notebook (the identical gap, same copied
    template) and ported here the same day since Regimen has the same lineage.

    `image` is a filename under health/img/ (an entry's hero) or "" for the
    branded default card, picked per `lang` since this publication has a
    Spanish twin. Card type follows the image's shape: no hero → the default
    card, large; a landscape hero ≥600px wide → large (X shows it wide,
    cropped toward 2:1); anything else (small, square, unreadable) → summary,
    whose square thumbnail shows the whole image rather than a cropped band."""
    if image:
        src = f"{SITE_URL}{BASE}/img/{image}"
        d = blogkit.img_dims(os.path.join(OUT, "img"), image)
        large = bool(d) and d[0] >= 600 and d[0] / d[1] >= 1.3
    else:
        src, large = (OG_IMAGE_ES if lang == "es" else OG_IMAGE), True
...
def _render_default_card(path, lang="en"):
    """Render the branded default og:image (health/img/og-default.png or its
    .es twin): the family's dark gradient + a framed card, in this
    publication's own dress — the wordmark with "Regimen"/"Régimen" in the
    accent teal and italic exactly as the header sets it, a divider, the
    tagline, and the path. Regenerated by every build so the asset can't
    drift from the wordmark; returns False (and the build keeps whatever file
    is already there) if Pillow or the fonts are missing, so a CI runner
    without Georgia still publishes."""
    try:
    ...
    accent = (63, 210, 168)                            # ACCENT, #3fd2a8

    W, H = 1200, 630
    img = Image.new("RGB", (W, H), (10, 22, 20))
    d = ImageDraw.Draw(img)
    top, bot = (10, 22, 20), (5, 12, 11)               # vertical gradient
    for y in range(H):
        t = y / (H - 1)
        d.line([(0, y), (W, y)], fill=(int(top[0] + (bot[0] - top[0]) * t),
                                       int(top[1] + (bot[1] - top[1]) * t),
                                       int(top[2] + (bot[2] - top[2]) * t)))
    glow = Image.new("RGBA", (W, H), (0, 0, 0, 0))     # soft teal glow near the top
    ImageDraw.Draw(glow).ellipse([W * 0.5 - 520, -380, W * 0.5 + 520, 300],
                                 fill=accent + (38,))
    glow = glow.filter(ImageFilter.GaussianBlur(120))
    img = Image.alpha_composite(img.convert("RGBA"), glow).convert("RGB")
    d = ImageDraw.Draw(img)
    d.rounded_rectangle([34, 34, W - 34, H - 34], radius=26, outline=(40, 84, 74), width=2)

    es = lang == "es"
    p1 = "El " if es else "The Librarian’s "
    p2 = "Régimen" if es else "Regimen"
    p3 = " del Bibliotecario" if es else ""
    wf, wfi = font("serif_b", 68), font("serif_bi", 68)
    w1 = d.textlength(p1, font=wf); w2 = d.textlength(p2, font=wfi)
    w3 = d.textlength(p3, font=wf) if p3 else 0
    sx = W / 2 - (w1 + w2 + w3) / 2
    d.text((sx, 205), p1, font=wf, fill=(247, 242, 226), anchor="lm")
    d.text((sx + w1, 205), p2, font=wfi, fill=accent, anchor="lm")
    if p3:
        d.text((sx + w1 + w2, 205), p3, font=wf, fill=(247, 242, 226), anchor="lm")

    d.rectangle([W / 2 - 48, 278, W / 2 + 48, 282], fill=accent)   # divider

    # TAGLINE_ES runs long — word-wraps the same way build_notebook.py's
    # card does, rather than the single centred line the Ledger's card uses.
    tagline = (TAGLINE_ES if es else TAGLINE).replace("'", "’")
    tf = font("sans", 26)
    lines, cur = [], ""
    for w in tagline.split():
        t = (cur + " " + w).strip()
        if cur and d.textlength(t, font=tf) > 980:
            lines.append(cur); cur = w
        else:
            cur = t
    if cur:
        lines.append(cur)
    ty = 348 - (len(lines) - 1) * 18
    for ln in lines:
        d.text((W / 2, ty), ln, font=tf, fill=(200, 206, 214), anchor="mm")
        ty += 36
    d.text((W / 2, ty + 40), "mistertranslation.com/health",
...
""" % {
        ...
        "feed": u["feed"],
        "locale": u["locale"],
        "social": _social_tags(e["title"], desc, e["hero"], lang),
...
def _shell(*, title, desc, url, body, active="", noindex=False, og_type="website",
           extra_css="", extra_js="", lang="en", alt_url=None, lang_href=None, image=""):
    ...
    `image` is a filename under health/img/ for a page with a picture of its
...
""" % {"lang": lang, "title": esc(title), "desc": esc(desc), "robots": robots, "url": url,
       ...
       "social": _social_tags(title, desc, image, lang),
...
/* Breadcrumb up to the hub (2026-09-21, Michael's ask, ported from the
   Notebook after he approved it there): "Mister Library › The Librarian's
   Regimen" — a page here had no way back to the root hub except the
   browser's back button. The hub mark below is a byte-for-byte copy of the
   <svg class="mark"> in the root index.html (that file is hand-written, not
   built, so there's nothing to import from — keep the two in step by hand
   if the hub mark ever changes). Same size (45px, header.hsm context), same
   gold gradient text-fill + drop-shadow halo as the hub's own <h1>/.mark, so
   the hub's identity travels with it rather than borrowing this
   publication's own mint accent. Always in English — the hub has no Spanish
   edition, so the crumb doesn't switch with `lang`. */
...
/* A tangential-but-relevant factual note (a study irony, a reference list) —
   kept visually distinct from both the evidence box (accent) and the opinion
   box (amber): a quieter slate tone signals "worth knowing" not "the answer". */
.entry .aside{margin:24px 0 0;padding:18px 24px;border-left:3px solid #4a6a8a;
  border-radius:0 10px 10px 0;background:#0b141d}
