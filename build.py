...
def nav_strip(book, num, position):
    ...
    if i < len(same) - 1:
        ...
        # At the last SHIPPED chapter, point "coming soon" at the chapter the book
        # is actually next in line for. If the previous chapter is also shipped we
        # are on a live sequential run, so num+1 is right (Jeremiah 22->23->24).
        # If it is not, this chapter was translated out of order (Psalms 1, then 23)
        # and num+1 would promise a chapter nobody is heading for -- so point at the
        # lowest unpublished chapter, which is where the sequence really resumes.
        if (num - 1) in same:
            nxt = num + 1
        else:
            nxt = next(c for c in range(1, BOOK_TOTAL.get(book, num) + 1) if c not in same)
