...
RUNDOWN_JS_VER = blogkit.asset_ver(OUT, "rundown.js")
...
def _social_tags(title, desc, image=""):
    ...
    verbatim from build_finance.py's `_social_tags` (the Ledger's own fix for
    the identical gap, 2026-09-16-ish). This publication's `<head>` never had
    it: every entry/shell template here hardcoded `twitter:card content="summary"`
    with no `og:image`/`twitter:image` at all, so a link shared to X/Slack/
    iMessage rendered as a bare text card even for an entry with a real hero
    photo — caught 2026-09-23 when Michael asked why an X preview had no image.

    `image` is a filename under notebook/img/ (an entry's hero) or "" for the
    branded default card. Card type follows the image's shape: no hero → the
    default card, large; a landscape hero ≥600px wide → large (X shows it wide,
...
def _render_default_card(path):
    """Render the branded default og:image (notebook/img/og-default.png): the
    family's dark gradient + a framed card, in this publication's own dress —
    the wordmark with "Notebook" in the periwinkle accent and italic exactly as
    the header sets it, a divider, the tagline, and the path. Regenerated by
    every build so the asset can't drift from the wordmark; returns False (and
    the build keeps whatever file is already there) if Pillow or the fonts are
    ...
    accent = (143, 163, 255)                           # ACCENT, #8fa3ff

    W, H = 1200, 630
    img = Image.new("RGB", (W, H), (13, 17, 28))
    d = ImageDraw.Draw(img)
    top, bot = (13, 17, 28), (7, 9, 17)                # vertical gradient
    for y in range(H):
        t = y / (H - 1)
        d.line([(0, y), (W, y)], fill=(int(top[0] + (bot[0] - top[0]) * t),
                                       int(top[1] + (bot[1] - top[1]) * t),
                                       int(top[2] + (bot[2] - top[2]) * t)))
    glow = Image.new("RGBA", (W, H), (0, 0, 0, 0))     # soft periwinkle glow near the top
    ImageDraw.Draw(glow).ellipse([W * 0.5 - 520, -380, W * 0.5 + 520, 300],
                                 fill=accent + (38,))
    glow = glow.filter(ImageFilter.GaussianBlur(120))
    img = Image.alpha_composite(img.convert("RGBA"), glow).convert("RGB")
    d = ImageDraw.Draw(img)
    d.rounded_rectangle([34, 34, W - 34, H - 34], radius=26, outline=(60, 68, 96), width=2)

    # wordmark: "The Librarian's " in cream, "Notebook" in accent italic — the
    # header's own two-tone treatment (.wm .em)
    wf, wfi = font("serif_b", 70), font("serif_bi", 70)
    p1, p2 = "The Librarian’s ", "Notebook"
    ...
    # TAGLINE runs ~110 chars — over 4x finance's, so unlike that card's single
    # centred line this one word-wraps (same helper as build.py's verse cards).
    tf = font("sans", 26)
    lines = []
    cur = ""
    for w in TAGLINE.replace("'", "’").split():
        t = (cur + " " + w).strip()
    ...
    for ln in lines:
        ...
        ty += 36
    d.text((W / 2, ty + 40), "mistertranslation.com/notebook",
...
def _rundown_box(data, entries=()):
    ...
    try:
        ...
        pass
    parts = ['<section class="rundown" data-rundown>',
    ...
    for sec in data["sections"]:
        ...
        label = sec.get("label", "")
        parts.append(_rdblock(label, "".join(lis)))
...
def _rdblock(label, lis_html):
    """One orderable/hideable rundown block: a section heading + its list,
    wrapped so rundown.js can move or hide the whole thing as a unit. Keyed
    by the section's own LABEL text (stable across days even though each
    day's items differ) rather than by position, so a saved reader
    preference like "Bitcoin first" survives from one day's box to the
    next."""
    return (
        ...
        '</span></h3>\n'
        '      <ul class="rdlist">%s</ul>\n'
        '    </div>'
    ) % (esc(label), esc(label), esc(label), esc(label), esc(label), lis_html)
...
def _tile(item):
    ...
    treatment The Librarian Abroad's cards use, reusing the hero file
    directly rather than a generated derivative (Abroad's own `_hero_img`
    does the same: `object-fit:cover` costs nothing to crop, and these hero
    photos are already sized for the web). No hero: the card renders
...
.rdblocks{display:flex;flex-direction:column}
.rdblock .rdsec{display:flex;align-items:center;justify-content:space-between;gap:10px}
.rdctl{display:inline-flex;gap:2px;flex:0 0 auto}
.rdctl button{background:none;border:none;cursor:pointer;padding:2px 5px;font-size:11px;
  color:#54617a;line-height:1;border-radius:4px}
.rdctl button:hover,.rdctl button:focus{color:__ACCENT__;background:#131c2b}
.rdtools{margin:14px 0 0;display:flex;flex-wrap:wrap;align-items:center;gap:10px;
  font-size:12.5px;color:#7f8fa6}
.rdreset{background:none;border:none;cursor:pointer;padding:0;font:inherit;color:#7f8fa6;
  text-decoration:underline}
.rdreset:hover,.rdreset:focus{color:__ACCENT__}
.rdhidden:empty{display:none}
.rdrestore{background:#131c2b;border:none;border-radius:999px;cursor:pointer;
  padding:2px 9px;font-size:12px;color:#a9b7c9;margin-left:4px}
.rdrestore:hover,.rdrestore:focus{color:__ACCENT__}
